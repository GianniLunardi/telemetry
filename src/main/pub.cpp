...
using namespace std;
using namespace zmqpp;                          // context, socket, message
...
int main(int argc, char *argv[])
{
    ...
    if (options_parsed.count("stress")) {
        ...
        std::ostringstream pld;         // like buffer of strings
        unsigned long int i = 0, n = 0;
        int n_pts = 200;
        while(_running) {
            pld.str("");        // initialize the string
            pld.clear();
            pld << "{"
                << "\"a\":" << ((rand() % 100) / 10.0) << ", "
                << "\"b\":" << ((rand() % 100) / 10.0 + 5) << ", "
